...
func WorkWithHtml2() {
	...
	if t, err := template.ParseFiles(filepath.Join(constants.HtmlTemplatesRootDir, "template1.html")); err == nil {
		...
		_ = t.Execute(os.Stdout, *data.Milk)
	} else {
...
func WorkWithHtml4() {
	...
	if err1 == nil && err2 == nil {
		...
		_ = t1.Execute(os.Stdout, *data.Milk)

		fmt.Println()
		utils.Printfln("Template name: %v", t2.Name())
		_ = t2.Execute(os.Stdout, *data.Milk)
...
func WorkWithHtml5() {
	...
	if t, err := template.ParseGlob(filepath.Join(constants.HtmlTemplatesRootDir, "*")); err == nil {
		_ = t.ExecuteTemplate(os.Stdout, "template1.html", *data.Milk)
		fmt.Println()
		_ = t.ExecuteTemplate(os.Stdout, "template2.html", *data.Milk)
	} else {
		utils.Printfln("Load template error: %v", err.Error())
	}
}
...
func WorkWithHtml10() {
	...
	if t, err := template.ParseFiles(filepath.Join(constants.HtmlTemplatesRootDir, "template6.html")); err == nil {
		_ = t.Execute(os.Stdout, data.Products)
	} else {
		utils.Printfln("Load template error: %v", err.Error())
	}
}
...
func WorkWithHtml14() {
	...
	if t, err := template.ParseFiles(
		...
		_ = t.ExecuteTemplate(os.Stdout, "mainTemplate", data.Products)
	} else {
		utils.Printfln("Load template error: %v", err.Error())
	}
}
